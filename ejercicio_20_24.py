...
for _ in range(CANTIDAD_VEHICULOS):
    tipo_vehiculo = randint(1, 4)          # 1‒4 al azar
    cabina_destino = randint(0, 2)         # cabina 0, 1 o 2 al azar
...
for i, cabina in enumerate(cabinas):
    nombres = []
    for _ in range(cabina.size()):
        tipo = cabina.move_to_end()
        nombres.append(VEHICULOS[tipo][0])
    print(f"  Cabina {i + 1} ({cabina.size()} vehículos): {', '.join(nombres)}")

#b. Atención de las cabinas + c. recaudación + d. conteo por tipo 

recaudacion   = [0, 0, 0]          # pesos recaudados por cabina
conteo        = [                  # conteo[cabina][tipo] → cantidad
    {1: 0, 2: 0, 3: 0, 4: 0},
    {1: 0, 2: 0, 3: 0, 4: 0},
    {1: 0, 2: 0, 3: 0, 4: 0},
]
...
cabina_ganadora = recaudacion.index(max_recaudacion) + 1   # base 1
